...
class FencerEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0).
        ...
        **kwargs,
        ...
        if self.wandb_log:
            ...
            self.eps_info = np.array(
                [0] * len(self.log_info), dtype=np.float32)
        ...
        self.last_obs = np.zeros(
            2 * 3 * len(self.target_point), dtype=np.float32)
        self.last_obs_opponent = np.zeros(
            2 * 3 * len(self.target_point), dtype=np.float32)
        ...
        if second_state_method == "manual" and second_state_model is None:
            raise ValueError(
                "You must specify the second_state_model when second_state_method is 'manual'")
    ...
    def outOfArena(self, agent=0):
        border0_vec = self.get_geom_com(
            f"{agent}_{self.center_point}") - self.get_geom_com(f"0_indicator")
        border1_vec = self.get_geom_com(
            f"{agent}_{self.center_point}") - self.get_geom_com(f"1_indicator")
    ...
    def reset_model(self):
        ...
        if self.enable_random and self.step_count > self.first_state_step + self.alter_state_step:
            qpos += self.np_random.uniform(low=-0.1,
                                           high=0.1, size=len(self.init_qpos))
            # qvel += self.np_random.uniform(low=-0.1,
            #                                high=0.1, size=len(self.init_qvel))
    ...
    def _get_obs(self, agent=0, version="v1"):
        ...
        if agent == 1:
            obs = np.concatenate(
                [obs, (this_obs - self.last_obs_opponent) * self.fps])
    ...
    def _get_opponent_action(self):
        if self.second_state_method == "manual" and self.oppent_model is None:
            print("second_state_method loading: ",
                  self.second_state_model_path)
